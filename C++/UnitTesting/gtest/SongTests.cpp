...
// for which there is a known result).  It takes more than five tests
// to test our five member functions.  When you write unit tests,
// you'll want to do likewise.
...
TEST(SongTests, truncateArtistsWhenChangedAfterCreationIfBarelyTooLong)
{
    ...
    Song s{"Muse", "Knights of Cydonia"};
    s.setArtist(longArtist);
    ASSERT_EQ(truncatedArtist, s.getArtist());
}
