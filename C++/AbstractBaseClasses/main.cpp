...
// deleteShape() deletes the object that s points to, so naturally s can't
// and shouldn't be const.  Because the Shape destructor is virtual, the
// correct destructor -- the one that matches the object's actual run-time
// type -- is called.
...
int main()
{
    ...
    // for_each must be a function, we had to write the deleteShape()
    // function to delete each object.
    std::for_each(v.begin(), v.end(), deleteShape);
